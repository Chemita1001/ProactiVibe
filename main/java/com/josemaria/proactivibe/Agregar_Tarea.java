...
public class Agregar_Tarea extends AppCompatActivity {
    ...
    private void GuardarDatos() {
        ...
        try {
            String[] fechaHora = Fecha.getText().toString().split(" ");
            ...
            int minutoInt = Integer.parseInt(hora[1]);

            calendario.set(año, mes - 1, dia, horaInt, minutoInt, 0);

            long tiempoNotificacion = calendario.getTimeInMillis();
            if (tiempoNotificacion <= System.currentTimeMillis()) {
                Toast.makeText(this, "La hora seleccionada ya ha pasado", Toast.LENGTH_SHORT).show();
                return;
            }

            programarNotificacion(calendario.getTimeInMillis(), Titulo.getText().toString(), Descripcion.getText().toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void programarNotificacion(long tiempoEnMilis, String titulo, String descripcion) {
